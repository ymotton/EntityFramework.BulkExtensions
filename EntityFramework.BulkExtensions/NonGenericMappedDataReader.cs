...
{
    class NonGenericMappedDataReader : IDataReader
    {
        private readonly IEnumerator _enumerator;

        public Dictionary<Type, Dictionary<int, Func<object, object>>> Selectors { get; private set; }
        ...
        public NonGenericMappedDataReader(Type baseType, IEnumerable enumerable, DbContext context)
        {
            ...
            Selectors = new Dictionary<Type, Dictionary<int, Func<object, object>>>();
            ...
            foreach (var kvp in tableMappings)
            {
                ...
                Selectors[entityType] = new Dictionary<int, Func<object, object>>();
                ...
                foreach (var col in propertyMaps)
                {
                    ...
                    {
                        var x = Expression.Parameter(parameterType, "x");

                        var expression =
                            Expression.Lambda<Func<object, object>>(
                                Expression.Convert(Expression.Constant(col.DefaultValue), typeof (object)), x);
                    ...
                    else
                    {
                        ...
                        Expression propertyExpression = parameterType == entityType
                        ...
                        var expression =
                            Expression.Lambda<Func<object, object>>(Expression.Convert(propertyExpression, typeof (object)),
                                x);
                        var selector = expression.Compile();
                        Selectors[entityType][currentIndex] = selector;
                    }
        ...
        private Dictionary<int, Func<object, object>> _currentEntityTypeSelectors;
